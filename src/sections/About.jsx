            ...
            <div className='h-full overflow-auto pr-2'>
              {
                selectBtn != 2 ? <div className='grid xl:grid-cols-2 xl:gap-4 lg:grid-cols-2 lg:gap-1 grid-cols-1 gap-4'>
                  {
                    {
                      0:EXPERIANCE_CARDS.map((item,index)=>(<ExperienceCard key={index} time={item.time} name={item.name} company={item.company}/>)),
                      1:EDUCATION_CARDS.map((item,index)=>(<EducationCard  key={index} time={item.time} name={item.name} company={item.company}/>)),
                ...
                : <div className='grid xl:grid-cols-4 xl:gap-4 lg:grid-cols-3 lg:gap-2 grid-cols-2 gap-1'>
                  {SKILLS.map((item,index)=>(<SkillsCard key={index} icon={item.icon} pop={item.pop} animation={SkillAnimation(0+((index+1)*0.08))}/>))}
                </div>
              }
            </div>
          </div>
